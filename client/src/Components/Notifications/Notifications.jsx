...
import { deleteNotificationApi, notificationData } from '../../Api/NotificationApi';
const Notifications = () => {
    ...
    <div className='notifiction-p'>
      {notification ? (notification.map((noti)=>(
        <div className="notification" key={noti._id}>
          <div className='noti-delete-icon' onClick={()=>deleteNoti(noti._id)}><MdOutlineDeleteOutline /></div>
          <h6>{noti.content} </h6>
      </div>
